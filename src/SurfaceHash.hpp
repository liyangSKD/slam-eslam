...
struct SurfaceHash
{
    typedef Buckets< std::vector<PoseParticle> > RoughnessHash;
    typedef Buckets< RoughnessHash > SlopeHash;

    boost::shared_ptr<SlopeHash> hash;
    ...
    {
	std::vector<PoseParticle> &ps( (*hash)[param.slope][param.roughness] );
	if( ps.size() > 0 )
	{
	...
	// roughness
	hash = boost::shared_ptr<SlopeHash>(
		new SlopeHash( 10, 0, 1.0, RoughnessHash( 10, 0, 1.0 ) ) );

	const double base = 0.5;

	std::vector<base::Vector3d> points;
	points.push_back( base::Vector3d( base/2.0, 0, 0 ) );
	points.push_back( base::Vector3d( -base/2.0, 0, 0 ) );
	points.push_back( base::Vector3d( base/2.0, -base, 0 ) );
	points.push_back( base::Vector3d( -base/2.0, -base, 0 ) );
	...
	for( size_t a = 0; a < angle_segments; a ++ )
	{
		...
		for( size_t n = 0; n < gridTemplate->getHeight(); n ++ )
		{
			...
			if( it != gridTemplate->endCell() )
			{
			    double zval = it->mean;
			    mean_z += zval;
			    gpoints.push_back( p + base::Vector3d( x, y, zval ) );
			}
		    }
		    mean_z /= gpoints.size();
			...
			PoseParticle particle( base::Vector2d( pose.x(), pose.y() ), angle + yaw_offset, pose.z() );

			(*hash)[params.slope][params.roughness].push_back( particle );
